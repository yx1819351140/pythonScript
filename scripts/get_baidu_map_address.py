...
def get_data():
    db_doris = pymysql.connect(host="10.32.48.2", user="z_smy", password="z_smy@0325", database="pro_qd_ztjc", charset="utf8")
    cursor_doris = db_doris.cursor()
    cursor_doris.execute("""SELECT 
	id,
	case 
	when label = '迁入' THEN add_before
	else add_after end
	address
FROM `pro_qd_dp_list_migrateinout` where c1=''""")
    data_list = cursor_doris.fetchall()
    for data in data_list:
        id = data[0]
        address = data[1]
        c1, c2 = get_address_info(address.replace('(', '').replace(')', '')[:20])
        print(id, address, c1, c2)
...
if __name__ == '__main__':
    # get_data()
    print(get_address_info1('山东省青岛市保税港区上海路34号三段五层5171（B） '))
