...
for _, row in df_all.iterrows():
    try:
        ...
        # 查询 company_id
        cursor.execute(
            f"SELECT company_id FROM beida_company WHERE company_name = '{company_name}'"
        )
        ...
        # 查询 industry_code
        cursor.execute(
            f"SELECT industry_code FROM industry_chain_detail WHERE name = '{chain_name}'"
        )
        result = cursor.fetchone()
        if not result:
        ...
        if operation == "增":
            # 插入数据
            cursor.execute(
                f"INSERT IGNORE INTO industry_chain_detail_company (industry_code, company_id) VALUES ('{industry_code}', '{company_id}')"
            )
            print(f"[新增] {company_name} {company_id} -> {chain_name} {industry_code}")
        elif operation == "删":
            # 删除数据
            cursor.execute(
                f"DELETE FROM industry_chain_detail_company WHERE industry_code = '{industry_code}' AND company_id = '{company_id}'"
            )
